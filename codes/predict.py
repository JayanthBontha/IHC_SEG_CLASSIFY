...
def save_combined_centroids_image(binary_masks):
    combined_centroid_image = np.zeros((256, 256), dtype=np.uint8)

    for instance in range(binary_masks.shape[2]):
        binary_mask = binary_masks[:, :, instance]

        # Find the centroid coordinates
        indices = np.where(binary_mask == 255)
        y_coordinates, x_coordinates = indices

        valid_indices = np.logical_and(np.isfinite(x_coordinates), np.isfinite(y_coordinates))
        x_coordinates = x_coordinates[valid_indices]
        y_coordinates = y_coordinates[valid_indices]

        if len(x_coordinates) == 0 or len(y_coordinates) == 0:
            continue

        centroid_x = int(np.mean(x_coordinates))
        centroid_y = int(np.mean(y_coordinates))

        # Set the centroid as a white pixel in the combined centroid image
        combined_centroid_image[centroid_y, centroid_x] = 255

    # Save the combined centroid image
    cv2.imwrite("./hello.png", combined_centroid_image)
...
def postprocess_masks(result, image, min_nuc_size=10):
    ...
    save_combined_centroids_image(result['masks'])
    return result
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            folder_name = os.path.join(folder_path, image_name)
            os.makedirs(folder_name, exist_ok=True)
            
            # Create a subfolder named "images" within the image folder
            subfolder_name = os.path.join(folder_name, "images")
            os.makedirs(subfolder_name, exist_ok=True)
            
            # Move the image to the subfolder
            image_path = os.path.join(folder_path, filename)
            new_image_path = os.path.join(subfolder_name, filename)
    ...
    model = get_model(config, model_path='./kaggle_bowl.h5')
    ...
    # Iterate through the subfolders in the folder
    for subfolder_name in os.listdir(folder_path):
        subfolder_path = os.path.join(folder_path, subfolder_name)

        # Check if the subfolder contains the "images" subfolder
        ...
        if os.path.exists(images_folder_path) and os.path.isdir(images_folder_path):
            # Move the image from the "images" subfolder back to the original folder
            ...
            # Remove the "images" subfolder and the original subfolder
